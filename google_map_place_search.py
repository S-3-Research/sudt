...
for location in tqdm(locations[:], desc="Locations"):
    ...
    if str(location) in visited:
        visited_count += 1
        continue

    # Perform the search
    treatment_centers = []
    response = gmaps.places(query=search_query, location=location[::-1], language='en-US', type='health', radius = 50000)
...
print(f"Visited {visited_count} locations")